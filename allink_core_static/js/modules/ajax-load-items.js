...
export function initAjaxLoadItemsTrigger() {
    ...
    $('.ajax-load-items').each(function(){
        $(this).on('click',function(e){
            ...
            // let me handle this ;)
            e.preventDefault();
            // special case: the dynamic grid (masonry.js) handles appending items
            if ($trigger.parents('.grid_dynamic-template').length > 0) {
                return false;
            }
            // default
            else {
                loadAjaxItems($trigger);
            }
        });
    });
...
export function loadAjaxItems($trigger,masonry_grid,masonry_instance) {
...
$(function(){
    ...
    // on page load
    initAjaxLoadItemsTrigger();

    // custom event
    $(window).on('initAjaxLoadItemsTrigger', function() {
        initAjaxLoadItemsTrigger();
    });
