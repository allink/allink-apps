...
class Brands(TranslationHelperMixin, TranslatedAutoSlugifyMixin, TranslatableModel, AllinkBaseModel):
    """
    Translations
    ...
    def preview_image(self):
        if self.brandsimage_set.count() > 0:
            return self.brandsimage_set.first().image

    @property
    def images(self):
        return self.brandsimage_set.all()


# APP CONTENT PLUGIN
class BrandsAppContentPlugin(AllinkManualEntriesMixin, AllinkBaseAppContentPlugin):
    """
...
class BrandsImage(AllinkBaseImage):
    brands = SortableForeignKey(Brands, verbose_name=_(u'Images'), help_text=_(u'The first image will be used as preview image.'), blank=True, null=True)
